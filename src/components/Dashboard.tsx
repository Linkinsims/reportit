...
export function Dashboard({ profile, org, onNavigate }: Props) {
  const stats = useQuery(api.reports.getDashboardStats, { organizationId: org._id });
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-gray-500 text-sm mt-1">Overview of incidents at {org.name}</p>
      </div>

      {/* Stat cards */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <StatCard label="Total Reports" value={stats.total} color="indigo" icon="📋" />
      ...
      {stats.highPriority > 0 && (
        <div className="mb-6 bg-red-50 border border-red-200 rounded-xl p-4 flex items-center gap-3">
          ...
          <div>
            <div className="font-semibold text-red-800">{stats.highPriority} High Priority Incident{stats.highPriority !== 1 ? "s" : ""} Need Attention</div>
            <div className="text-sm text-red-600">These require immediate review</div>
          </div>
          <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border p-5">
          <h3 className="font-semibold text-gray-900 mb-4">Reports This Week</h3>
          ...
          <div className="flex items-center gap-6">
            <DonutChart
              data={[
                ...
                { label: "In Progress", value: stats.byStatus.in_progress, color: "#F59E0B" },
                { label: "Resolved", value: stats.byStatus.resolved, color: "#10B981" },
              ]}
              total={stats.total}
            />
...
function DonutChart({ data, total }: { data: Array<{ label: string; value: number; color: string }>; total: number }) {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="flex-shrink-0">
      ...
      <text x={size / 2} y={size / 2 + 5} textAnchor="middle" className="text-xs font-bold" fill="#111827" fontSize="14" fontWeight="bold">
        {total}
      </text>
    </svg>
  );
