...
export function OnboardingFlow() {
  ...
  const [createForm, setCreateForm] = useState({ name: "", slug: "", displayName: "", role: "admin" as Role });
  const [joinForm, setJoinForm] = useState({ slug: "", displayName: "", role: "employee" as Role });
  ...
  async function handleJoin(e: React.FormEvent) {
    ...
    try {
      await joinOrg({
        slug: joinForm.slug.toLowerCase(),
      ...
      <div className="w-full max-w-md">
        ...
        {step === "choice" && (
          <div className="bg-white rounded-2xl shadow-sm border p-6 space-y-3">
            ...
            >
              <div className="font-semibold text-gray-900 group-hover:text-indigo-700">Create Organization</div>
              <div className="text-sm text-gray-500 mt-0.5">Start fresh with a new workspace</div>
            </button>
            <button
            ...
            >
              ...
              <div className="text-sm text-gray-500 mt-0.5">Enter an existing workspace slug</div>
            </button>
          </div>
        )}
        ...
        {step === "create" && (
          <div className="bg-white rounded-2xl shadow-sm border p-6">
            <button onClick={() => setStep("choice")} className="text-sm text-gray-500 hover:text-gray-700 mb-4 flex items-center gap-1">
              ← Back
            </button>
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  required
                />
                <p className="text-xs text-gray-400 mt-1">Share this slug with teammates to join</p>
              </div>
              <div>
            ...
            <form onSubmit={handleJoin} className="space-y-4">
              ...
              <div>
                ...
                <select
                  className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm"
                  value={joinForm.role}
                  onChange={(e) => setJoinForm({ ...joinForm, role: e.target.value as Role })}
                >
                  <option value="employee">Employee</option>
                  <option value="manager">Manager</option>
                  <option value="admin">Admin</option>
